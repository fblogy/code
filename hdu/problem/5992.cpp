...
struct node{
	int d[2], l, r, mi[2], ma[2], minp, p, id;//sum, val
...
struct KDTree{
	...
	inline void check(const node &v, const node &p) {
		...
		if (dis < now || (dis == now && p.id < ans.id)) {
			now = dis;
			ans = p;
		}
		return;
...
int main(){
	...
	rep(cas, 0, T) {
		...
		rep(i, 1, n+1) {
			...
			cin >> a[i].p; a[i].id = i;a[i].minp = a[i].p;
		}
		//rep(i, 1, n+1) b[i] = a[i];
		...
		rep(i, 1, m+1) {
			...
			now = INF;
			kdt.ask(v, kdt.root);
			cout << ans.d[0] << " " << ans.d[1] << " " << ans.p << endl;
		}
	}
}
