...
int main() {
	...
	while(scanf("%d%d%d", &n, &q, &m) == 3) {
		...
		for(int msk = 0; msk < 1 << m; ++msk) {
			for(int i = 0; i < n; ++i) {
				int cur = i < m ? 1 << i : 0;
				if(cur && !(msk & cur))continue;
				int rem = msk ^ cur, lbt = rem & -rem;
				if (rem) for(int msk2 = (rem - 1) & rem; msk2; msk2 = (msk2 - 1) & rem) if(msk2 & lbt) {
					 	int u = msk2 | cur, v = (rem ^ msk2) | cur;
						g[msk][i].upd(f[u][i].merge(g[v][i]));
					}
				if(g[msk][i].val < n) adt[g[msk][i].val].push_back(i);
			}
			tot = 0;
		...
		Info ans = (Info){n, 0};
		for(int i = 0; i < n; ++i) ans.upd(g[(1 << m) - 1][i]);
		if(ans.val < n) {
